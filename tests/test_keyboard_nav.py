...
def test_tab_key_cycles_improve_submode(qapp):
  ...
  start = tw._improve_submode
  # Need a lesson so keyPressEvent does not early-return.
  ...
  assert tw._improve_submode != start or start == 0
  # At least one cycle happened (may skip empty oblivion; still should move or stay if only one).
