...
- **Session clock** (`amphetype/session_timer.py`): top-right overlay on the main window (`H:MM:SS`). Counts elapsed time only while the app window is focused; pauses when another window is frontmost. Unaffected by lesson pause, settings, or tab changes.
