...
- **Pause (ESC)**: during an active lesson, ESC pauses (does not cancel). WPM timing freezes while paused (`RunStats.pause` / `resume`). The lesson canvas greys out with **Continue**, **New**, and **Restart** (stacked); ESC again or **Continue** resumes. **New** loads another exercise for the current mode (normal / book / weakspot). **Restart** resets the lesson to the beginning. No on-screen ESC hint.
