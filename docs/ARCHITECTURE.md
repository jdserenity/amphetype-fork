...
- **Word progress feedback** (`amphetype/word_progress.py`): at lesson load, all-time word WPM baselines and raw timing samples are fetched for words in the text. Green highlight, grey `+Nwpm` badges, and end-of-run summary all use the same rule: merging this run's whole-word sample into the historical median pool must raise median WPM by ≥1. After the run: grey boxes show that shift when &gt;0 (`+Nwpm`, or `+N` on narrow words). Performance Analysis **words** tab adds an **Improved** column (current window median WPM minus WPM from the first recorded sample) and a **most improved** sort option.
