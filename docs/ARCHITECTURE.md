...
- **Stats** sub-tab: double-click a row drills that type target; **Drill worst 3** (top damage); **Drill 3 oblivion** (random three with WPM `<30` for current keys/trigrams/words). Switches to Typer in weakspot mode with a half-length lesson built only from those targets (`build_focus_lesson` in `WeakSpotLessons.py`); focus lessons emit each target word with the exact `data` string from Analysis (no re-capitalization). Finishing replays the same drill; click **weakspot** in the footer or leave weakspot and return to resume mixed weakspot lessons. Focus drills write no stats (no `result` or `statistic` rows). Word heatmap colors use case-sensitive stat keys; focus-drill targets also use the Analysis WPM captured at drill start.
