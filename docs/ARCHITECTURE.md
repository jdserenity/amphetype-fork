...
- **Word progress feedback** (`amphetype/word_progress.py`): at lesson load, all-time word WPM baselines are fetched for words in the text. Words beat baseline by ≥1 whole WPM turn green while typing. After the run: grey boxes over improved words show baseline delta (`+Nwpm`, or `+N` on narrow words); run WPM uses the same whole-word timing as word stats in `statistic`. Summary above the typer — “You improved on *N* out of *M* words at an average of +*G*wpm!” (*N*/*G* green if &gt;0, red if 0); new-word count in orange when &gt;0. Lesson stays on screen until ENTER; footer adds “Press ENTER to start the next exercise.” Focus drills show the same summary plus “Drill only — stats were not saved.” Default practice mode is **improve** (`practice_mode`: 0).
