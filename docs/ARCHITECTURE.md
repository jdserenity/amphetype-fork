...
- **Speed heatmap** (off by default): footer **heatmap** click-toggle (white = on, grey = off); when on, one **words** / **trigrams** / **chars** label (click to cycle) plus single-line WPM legend (`<32` purple oblivion, `32–54`, `55–77`, `78–99`, `100+`). Untyped text with stats gets foreground color from `statistic` (all-time query; same counted-row rules as Performance Analysis). Case-sensitive key lookup. Word stats include all lengths (no min length). Trigram mode paints non-overlapping 3-char blocks; contested spans use highest damage (`count·time²·(1+misses/count)`, same as Performance Analysis). Logic: `amphetype/speed_heatmap.py`.
- **Word progress feedback** (`amphetype/word_progress.py`): at lesson load, all-time word WPM baselines are fetched for words in the text. Typed words beat baseline by ≥1 whole WPM get progress green (`#00e676`) on the lesson (works alongside heatmap). After each run: summary above the typer — “You improved on *N* out of *M* words!” (*N* green if &gt;0, red if 0); “You typed *K* unique new words!” only when *K*&gt;0. Focus drills show the same summary plus “Drill only — stats were not saved.” Improve mode runs save weakspot-tagged word stats and use the same UI.
