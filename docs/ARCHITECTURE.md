...
- **Pause (ESC)**: during an active lesson, ESC pauses (does not cancel). WPM timing freezes while paused (`RunStats.pause` / `resume`). The lesson canvas greys out with **Restart** and **Continue**; ESC again or **Continue** resumes. **Restart** resets the lesson to the beginning. No on-screen ESC hint.
