...
- **Word progress feedback** (`amphetype/word_progress.py`): at lesson load, all-time word WPM baselines and raw timing samples are fetched for words in the text. Words beat baseline by ≥1 whole WPM this run turn green while typing. After the run: grey boxes show median-pool WPM shift when &gt;0 (`+Nwpm`, or `+N` on narrow words). Summary counts only words whose median would move. Performance Analysis **words** tab adds an **Improved** column: current window median WPM minus WPM from the first recorded sample (all-time, from existing `statistic` rows).
