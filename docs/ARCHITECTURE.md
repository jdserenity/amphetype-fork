...
- **Word progress feedback** (`amphetype/word_progress.py`): at lesson load, all-time word WPM baselines and raw timing samples are fetched for words in the text. Words beat baseline by ≥1 whole WPM this run turn green while typing. After the run: grey boxes show how much the word’s median WPM would rise if this sample were merged into history (`+Nwpm`, or `+N` on narrow words); green still requires a strong rep vs current baseline. Summary — “You improved on *N* out of *M* words at an average of +*G*wpm!” (*N*/*G* green if &gt;0, red if 0); new-word count in orange when &gt;0. Lesson stays on screen until ENTER. Default practice mode is **improve** (`practice_mode`: 0).
