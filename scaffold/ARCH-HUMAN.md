...
**Keyboard (macOS):** **Tab** cycles improve submode (when you’re in improve). **⌘⌥← / ⌘⌥→** cycle practice mode (improve · corpus · book). **⌘⇧[ / ⌘⇧]** cycle the top strip as one sequence (Typer → Performance Analysis → General Options → Typer Options → Sources → …). Preferences sub-tabs appear on that strip next to Preferences.
