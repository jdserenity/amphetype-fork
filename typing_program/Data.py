...
from typing_program.stats_query import STAT_OMIT_DISCOUNTED  # noqa: F401 — re-export
...
class AppDatabase(sqlite3.Connection):
  def __init__(self, *args):
    super(AppDatabase, self).__init__(*args)
...
create view text_source as
  ...
  def executemany_(self, *args):
    super(AppDatabase, self).executemany(*args)
...
# GLOBAL
DB = sqlite3.connect(dbname,5,0,"DEFERRED",False,AppDatabase)

def switchdb(nn):
  global DB
  DB.commit()
  try:
    nDB = sqlite3.connect(nn,5,0,"DEFERRED",False,AppDatabase)
    DB = nDB
