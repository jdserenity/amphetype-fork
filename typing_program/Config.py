...
class AppSettings(FSettings, metaclass=SettingsMeta):
  change = pyqtSignal()
  DATA_DIR = DATA_DIR
...
from typing_program.typing_sounds import TypingSoundPlayer, format_sound_label, list_sound_ids
...
class TyperSoundOptions(QGroupBox):
  def __init__(self, S, *args, **kwargs):
    ...
    self._no_sounds_btn = AppButton('No sounds', self._clear_all_sounds)
    self._sync_preview()
    self.setLayout(FBoxLayout([
      'Keystroke sounds while typing. Each list only shows matching files (type-*, error-*).',
      [self._no_sounds_btn, None],
      ['Correct keystroke', self._type_combo, AppButton('Preview', self._preview_type), None],
      ['Error keystroke', self._err_combo, AppButton('Preview', self._preview_error), None],
...
class GeneralOptions(QWidget):
  def __init__(self, *args, **kwargs):
    ...
    self.setLayout(AppBoxLayout([
      ...
      "(In order selects the next text after the one you completed last. Easy/difficult estimates your WPM for a random sample and picks the fastest/slowest.)\n",
