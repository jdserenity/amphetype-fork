  ...
  def __init__(self, *args):
    ...
    self.setLayout(AppBoxLayout([
      ["The database is essentially your 'profile'. It stores all your statistics, the texts you've imported, the lessons you've generated and so forth.", None],
        [AppButton("Update", self.update), 150,
          ([["Current database:", SettingsEdit("db_name")],
          ["For the database change to take effect you need to restart Typing Program. You can also specify a database name at the command line with the '--database=&lt;file&gt;' switch.\n"]
          ], 1)
        ], #AppButton("Import", self.importdb), "external DB file"]]],
        self.stats_,
        None,
        ["DELETE all statistics and results from:",
         AppButton('Last Minute', lambda: self.delete_last(60.0)),
         AppButton('Last Hour', lambda: self.delete_last(60.0*60.0)),
         AppButton('Last 24 Hours', lambda: self.delete_last(60.0*60.0*24.0)),
         None],
        None,
        ...
        ["Group data older than", SettingsEdit('group_month'), "days into months, data older than",
          ...
          None],
        [AppButton("Go!", self.cleanup), None],
        [self.progress_],
        None
      ]))
