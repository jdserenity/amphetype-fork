...
from amphetype.QtUtil import AmphButton, WWLabel
...
from amphetype.gutenberg.fetch import estimate_import_seconds, write_book_file
...
class GutenbergBrowser(QWidget):
  ...
  def __init__(self, parent=None):
    ...
    self._stale = WWLabel('')
    self._stale_btn = AmphButton('Update', self._start_catalog_update)
    ...
    self._search_btn = AmphButton('Search', self._run_search)

    self._results = QListWidget()
    self._results.itemDoubleClicked.connect(lambda _: self._import_selected())

    self._import_btn = AmphButton('Import selected', self._import_selected)
